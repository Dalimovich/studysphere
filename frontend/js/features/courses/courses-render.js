...
function _openedCount(courseId, total) {
    if (!total)
        return 0;
    try {
...
function _aiSessionCount(courseId) {
    try {
...
function _lastOpenedLabel(courseId) {
    let ts = 0;
    try {
        ts = Number(localStorage.getItem('ss_lastopen_' + courseId) || 0);
    }
    catch { /* skip */ }
    if (!ts)
        return 'Not started';
...
function _safePercent(done, total) {
    if (!total || total <= 0)
        return 0;
    return Math.min(100, Math.round((done / total) * 100));
}
export function computeCourseProgress(courseId, files) {
    return _computeProgress(courseId, files);
}
function _computeProgress(courseId, files) {
    ...
    if (aiReviewProgress !== null)
        parts.push({ value: aiReviewProgress, weight: 0.15 });
...
function _buildStatPill(label, value, accent) {
    return ('<div class="sd-stat-pill" style="--sd-stat-accent:' + accent + '">' +
...
export function sdRenderCourses(state) {
    ...
    sem.courses.forEach((c, i) => {
        ...
        const _pickCourseIcon = (name) => {
            const n = name.toLowerCase();
            if (/mechan|statik|dynamik/.test(n))
                return '📘';
            if (/konstru|cad|design/.test(n))
                return '📐';
            if (/maschin|antrieb|getriebe/.test(n))
                return '⚙️';
            if (/fertig|product|manufact/.test(n))
                return '🏭';
            if (/elektro|electric|schalt/.test(n))
                return '⚡';
            if (/mathe|math|analysis/.test(n))
                return '📊';
            if (/inform|programm|code/.test(n))
                return '💻';
            if (/chem/.test(n))
                return '🧪';
            if (/physi/.test(n))
                return '🔬';
            if (/sprach|deutsch|german|englisch/.test(n))
                return '🗣';
            return '📙';
        };
        ...
        const progressBlock = count > 0
            ...
            : '<div class="sd-course-empty-msg">' +
                ...
                '</div>' +
                '</div>';
        const lastOpenedHtml = '<span class="sd-course-chip sd-course-chip-time">' +
        ...
        const badgeEl = card.querySelector('[data-file-badge]');
        if (!liveCount && badgeEl)
            _hydrateCardCount(c.id, badgeEl);
        if (count === 0)
            card.classList.add('sd-course-card-empty');
        const delBtn = card.querySelector('.sd-del-btn');
        if (delBtn) {
            delBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                if (!window.confirm('Delete ' + c.name + '?\n\nThis will remove the course from your list. Uploaded files in Supabase storage are not deleted by this action.'))
                    return;
...
function _updateHeroStats(state, data) {
    const semLbl = document.getElementById('sdHeroSemLabel');
    const semBtnLbl = document.getElementById('sdSemLabel');
    if (semLbl && semBtnLbl)
        semLbl.textContent = semBtnLbl.textContent || '';
    const semColor = state.SEMS[state.sdActiveSemId]?.color;
    if (semColor) {
        document.querySelectorAll('#sdHeroStats .sd-hero-stat-dot').forEach((el) => {
    ...
    if (courseLbl)
        courseLbl.textContent = data.length + ' course' + (data.length === 1 ? '' : 's');
    const totalFiles = data.reduce((s, d) => s + d.progress.files, 0);
    if (filesLbl)
        filesLbl.textContent = totalFiles + ' file' + (totalFiles === 1 ? '' : 's');
    ...
    const avg = tracked.length
        ...
        : 0;
    if (progressLbl)
        progressLbl.textContent = avg + '% avg progress';
}
function _renderNextStepsBelowGrid(data) {
    ...
    if (unread) {
        ...
        suggestions.push({
            ...
            action: () => {
                if (typeof window.openCourse === 'function')
                    window.openCourse(unread.course);
            },
        });
    }
    // Suggest AI chat on a course that has files but no chat yet
    const noChat = data.find((d) => d.progress.files > 0 && d.progress.aiSessions === 0);
    if (noChat) {
        suggestions.push({
            id: 'ask-ai',
            icon: '🤖',
            ...
            action: () => {
                if (typeof window.openCourse === 'function')
                    window.openCourse(noChat.course);
            },
        });
    }
    ...
    const grid = wrap.querySelector('.sd-next-steps-grid');
    if (grid) {
