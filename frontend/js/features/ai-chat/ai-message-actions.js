...
export function copyBubble(btn) {
    ...
    const bubble = body?.querySelector('.ai-bubble');
    if (!bubble)
        return;
...
export function regenMsg(btn) {
    ...
    if (typeof window.askAI === 'function')
        window.askAI(q, true, { forceRefresh: true });
}
export function bindMessageActionButtons(wrap) {
    if (!wrap)
        return wrap;
    wrap.querySelectorAll('.msg-action-btn[data-action="copy"]').forEach((btn) => {
        btn.addEventListener('click', () => copyBubble(btn));
    });
    wrap.querySelectorAll('.msg-action-btn[data-action="regen"]').forEach((btn) => {
...
export function addBotMsg(text) {
    ...
    const botBubble = wrap.querySelector('.ai-bubble.bot');
    if (botBubble) {
        ...
        if (window._ssEnsureKatex) {
            window
                ._ssEnsureKatex()
                .then(() => {
                if (window._renderMath && botBubble)
                    window._renderMath(botBubble);
            })
                .catch(() => { });
        }
    }
    ...
    if (msgBody && typeof window._aiResponseActions === 'function') {
        const actions = window._aiResponseActions(text, 'panel');
        if (actions)
...
export function addUserMsg(text) {
    ...
    if (!aiMsgs)
        return null;
    if (typeof window._statsTrackAI === 'function')
        window._statsTrackAI();
    const wrap = document.createElement('div');
...
export function setAiChipsVisible(v) {
    const el = document.querySelector('.ai-chips');
    if (el)
        el.style.display = v ? '' : 'none';
    if (!v) {
        const lbl = document.getElementById('aiFileLabel');
        if (lbl)
            lbl.textContent = window._t ? window._t('ai_ready') : 'Ready to help';
        const chip = document.getElementById('aiFileChip');
        if (chip)
            chip.classList.add('empty');
        const chipName = document.getElementById('aiFileChipName');
        if (chipName)
            chipName.textContent = window._t ? window._t('no_file_open') : 'No file open';
    }
}
export function serializeChatDOM() {
    ...
    aiMsgs.querySelectorAll('.ai-msg-wrap').forEach((wrap) => {
        if (wrap.classList.contains('typing-wrap'))
            return;
        if (wrap.getAttribute('data-restored') === 'true')
            return;
...
//# sourceMappingURL=ai-message-actions.js.map