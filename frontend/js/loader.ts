    ...
    loadScript('js/dependencies.js', 'dependencies-script')
      ...
      .then(() => {
        ...
        void Promise.all(featurePromises).then(() => {
          // Old js/ai.js engine is no longer loaded — the chatbot-new shell
          // owns all AI calls now. Still fire ss-ready so anything that
          // listened for the boot completion keeps working.
          if (SS) {
