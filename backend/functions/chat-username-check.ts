...
export const handler = async (event: NetlifyEvent): Promise<LambdaResponse> => {
  ...
  const username = String((event.queryStringParameters && event.queryStringParameters.username) || '')
    .trim().toLowerCase();
  if (!username || username.length < 3 || !/^[a-z0-9_]+$/.test(username)) return fail(400, 'Invalid username');
